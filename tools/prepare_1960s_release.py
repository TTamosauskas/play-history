...
DECADE = 1960
EXPECTED_TOTAL = 132
EXPECTED_BY_YEAR = {1960:12,1961:7,1962:5,1963:5,1964:18,1965:15,1966:12,1967:18,1968:21,1969:19}
...
def patch_runtime() -> None:
    ...
    line = "    ...Array.from({length: 10}, (_, i) => `context_1960s_${1960 + i}.json`),\n"
    if "context_1960s_" not in text:
        text = text.replace(marker, marker + line, 1)
    if f"const VERSION = '{VERSION_NEW}';" not in text or "context_1960s_" not in text:
...
def main() -> None:
    ...
    patch_runtime()
    patch_index()
    print(f"OK: prepared 1960s release candidate v{VERSION_NEW}; {EXPECTED_TOTAL} tracks")
